...
-- Remove any existing default constraint completely
ALTER TABLE public.profiles ALTER COLUMN role DROP DEFAULT;

-- Convert text values to proper enum values before type change
UPDATE public.profiles SET role = 
  CASE 
    WHEN role = 'admin' THEN 'admin'
    WHEN role = 'editor' THEN 'editor'  
    WHEN role = 'moderator' THEN 'editor'
    ELSE 'user'
  END;

-- Change column type to enum with explicit casting
ALTER TABLE public.profiles ALTER COLUMN role TYPE user_role USING role::user_role;

-- Set the default value as enum
