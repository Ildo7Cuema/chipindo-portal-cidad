-- Adicionar acesso por setor aos perfis de utilizadores
-- Portal Cidadão de Chipindo

-- Adicionar coluna setor_id à tabela profiles
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS setor_id UUID REFERENCES setores_estrategicos(id) ON DELETE SET NULL;

-- Atualizar a constraint de role para incluir os novos roles por setor
DO $$ 
BEGIN
    IF EXISTS (
...
-- Adicionar nova constraint que permite roles por setor
...
-- Criar índice para melhorar performance de consultas por setor
CREATE INDEX IF NOT EXISTS idx_profiles_setor_id ON profiles(setor_id);

-- Criar função para verificar acesso por setor
...
-- Criar função para obter o setor do utilizador
