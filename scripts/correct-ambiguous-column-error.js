...
async function correctAmbiguousColumnError() {
  try {
    console.log('🔧 Corrigindo erro de ambiguidade na coluna current_participants...\n');
    ...
    // 2. Testar a função atual para confirmar o erro
    console.log('\n2️⃣ Confirmando erro atual...');
    ...
    const testRegistration = {
      ...
      p_participant_email: 'teste.correcao.ambigua@email.com',
    ...
    try {
      ...
      if (functionError) {
        if (functionError.message.includes('column reference "current_participants" is ambiguous')) {
          console.log('✅ Erro confirmado: ambiguidade na coluna current_participants');
    ...
    // 3. Fornecer a correção
    console.log('\n3️⃣ Fornecendo correção para o erro...');
    
    console.log('💡 Execute este código SQL no Supabase SQL Editor para corrigir o erro:');
    console.log('\n' + '='.repeat(80));
    console.log(`
-- Correção da função register_for_event para resolver ambiguidade na coluna current_participants
...
$$;
    ...
    console.log('   1. Renomeei a variável `current_participants` para `event_current_participants`');
    console.log('   2. Uso a variável `event_current_participants` no UPDATE');
    console.log('   3. Isso elimina a ambiguidade entre variável e coluna');

    console.log('\n🎯 **Após aplicar a correção:**');
    console.log('   1. O modal de inscrição funcionará corretamente');
    console.log('   2. O erro 400 (Bad Request) será resolvido');
    console.log('   3. As inscrições serão salvas no banco de dados');

    console.log('\n✅ **Para testar após a correção:**');
    console.log('   1. Execute o código SQL acima no Supabase');
    console.log('   2. Teste o modal de inscrição na página de eventos');
    console.log('   3. Verifique se a inscrição é salva corretamente');
