...
console.log('🚀 Aplicando migração das inscrições em eventos diretamente...\n');
...
async function applyMigration() {
  try {
    ...
    // 7. Verificar se a tabela events tem os campos necessários
    console.log('\n7️⃣ Verificando tabela events...');
    
    const checkEventsSQL = `
      SELECT column_name, data_type 
      FROM information_schema.columns 
      WHERE table_name = 'events' 
      AND column_name IN ('max_participants', 'current_participants');
    `;
    
    const { data: eventsColumns, error: checkError } = await supabase.rpc('exec_sql', { sql: checkEventsSQL });
    
    if (checkError) {
      console.log('⚠️  Não foi possível verificar a tabela events');
    } else {
      console.log('✅ Tabela events verificada');
    }
