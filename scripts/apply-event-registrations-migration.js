...
console.log('🚀 Aplicando migração das inscrições em eventos...\n');
...
async function applyEventRegistrationsMigration() {
  try {
    ...
    const indexesSQL = `
      ...
      CREATE UNIQUE INDEX IF NOT EXISTS idx_event_registrations_unique ON event_registrations(event_id, participant_email);
    `;
    ...
    if (functionsError) {
      console.error('❌ Erro ao criar funções:', functionsError.message);
    } else {
    ...
    if (sampleDataError) {
      console.error('❌ Erro ao inserir dados de exemplo:', sampleDataError.message);
    } else {
      console.log('✅ Dados de exemplo inseridos com sucesso');
    }

    // 6. Atualizar contadores de participantes
    console.log('\n6️⃣ Atualizando contadores de participantes...');
    
    const updateCountersSQL = `
      UPDATE events 
    ...
    const { error: countersError } = await supabase.rpc('exec_sql', { sql: updateCountersSQL });
    
    if (countersError) {
      console.error('❌ Erro ao atualizar contadores:', countersError.message);
    } else {
