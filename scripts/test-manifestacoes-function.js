...
async function testManifestacoesFunction() {
  console.log('🧪 Testando função get_manifestacoes...\n');

  try {
    // Testar função get_manifestacoes
    console.log('📝 Testando busca de manifestações...');
    const { data: manifestacoes, error } = await supabase.rpc('get_manifestacoes', {
      p_search: null,
      p_categoria: null,
      p_status: null,
    ...
    if (error) {
      console.error('❌ Erro na função get_manifestacoes:', error);
      ...
      if (fixError) {
        ...
        if (testError) {
          ...
          if (testData && testData.length > 0) {
            ...
            testData.slice(0, 3).forEach((man, index) => {
      ...
      console.log('✅ Função funcionando corretamente:', manifestacoes?.length || 0, 'manifestações');
