...
import { TruckIcon, BusIcon, BuildingIcon, MapPinIcon, PhoneIcon, MailIcon, ArrowRightIcon, CheckCircleIcon, RouteIcon, XIcon } from "lucide-react";
    ...
    useEffect(() => {
        ...
        const loadSetor = async () => {
            try {
                ...
                const data = await getSetorBySlug('transporte');
                if (!isMounted) return;
                if (data) {
                        ...
                        <div className="overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0 scrollbar-hide pb-1">
                            <TabsList className="inline-flex sm:flex sm:flex-wrap w-max sm:w-full gap-1.5 sm:gap-2 p-1.5 sm:p-2 bg-muted/60 backdrop-blur-sm rounded-xl shadow-sm">
                                <TabsTrigger value="programas" className="min-h-[48px] sm:min-h-[44px] flex-shrink-0 sm:flex-1 min-w-[110px] sm:min-w-0 text-sm font-medium px-4 py-3 rounded-xl data-[state=active]:shadow-md transition-all touch-manipulation">
                                    <RouteIcon className="w-4 h-4 mr-2 sm:hidden" /><span className="truncate">Programas</span>
                                </TabsTrigger>
                        ...
                        <TabsContent value="programas" className="mt-5 sm:mt-8 lg:mt-10 animate-in fade-in-50 duration-300">
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
                                {setor.programas.map((programa, index) => (
                                    <Card key={index} className="group relative flex flex-col hover:shadow-2xl hover:shadow-sky-100/50 transition-all duration-300 hover:-translate-y-1 border-0 bg-white dark:bg-gray-800/90 overflow-hidden rounded-xl">
                                        <div className="h-1.5 sm:h-1 bg-gradient-to-r from-sky-500 to-blue-600 w-full" />
                                        <CardHeader className="p-4 sm:p-5 lg:p-6 pb-3 sm:pb-4">
                                            <CardTitle className="flex items-start gap-3 text-base sm:text-lg group-hover:text-sky-600 transition-all">
                                                <div className="p-2.5 sm:p-3 rounded-xl bg-sky-50 dark:bg-sky-900/30 text-sky-600 group-hover:bg-sky-600 group-hover:text-white transition-all shrink-0"><TruckIcon className="w-5 h-5" /></div>
                                        ...
                                        <CardContent className="flex-1 flex flex-col p-4 sm:p-5 lg:p-6 pt-0 space-y-4">
                                            ...
                                            <div className="pt-4 border-t border-gray-100 dark:border-gray-700 mt-auto">
                                                <Button className="w-full min-h-[48px] sm:min-h-[44px] bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white font-medium rounded-xl transition-all active:scale-[0.98] touch-manipulation" onClick={() => { setProgramaSelecionado(programa.titulo); setOpenInscricaoPrograma(true); }}>
                                    ...
                                    <Card key={index} className="group relative flex flex-col hover:shadow-2xl hover:shadow-orange-100/50 transition-all duration-300 hover:-translate-y-1 border-0 bg-white dark:bg-gray-800/90 overflow-hidden rounded-xl">
                                        ...
                                        <CardHeader className="p-4 sm:p-5 lg:p-6 pb-3 sm:pb-4">
                                            <div className="flex items-center justify-between mb-3 gap-2">
                                                ...
                                                <Badge variant="outline" className="border-orange-200 text-orange-700 text-xs px-2.5 py-1">Terminal/Rota</Badge>
                                            </div>
