...
import { ShieldCheckIcon, SearchIcon, BuildingIcon, MapPinIcon, PhoneIcon, MailIcon, ArrowRightIcon, CheckCircleIcon, ClipboardCheckIcon, XIcon } from "lucide-react";
    ...
    useEffect(() => {
        ...
        const loadSetor = async () => {
            try {
                ...
                const data = await getSetorBySlug('fiscalizacao');
                if (!isMounted) return;
                if (data) {
                        ...
                        <div className="overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0 scrollbar-hide pb-1">
                            <TabsList className="inline-flex sm:flex sm:flex-wrap w-max sm:w-full gap-1.5 sm:gap-2 p-1.5 sm:p-2 bg-muted/60 backdrop-blur-sm rounded-xl shadow-sm">
                                <TabsTrigger value="programas" className="min-h-[48px] sm:min-h-[44px] flex-shrink-0 sm:flex-1 min-w-[110px] sm:min-w-0 text-sm font-medium px-4 py-3 rounded-xl data-[state=active]:shadow-md transition-all touch-manipulation">
                                    <ClipboardCheckIcon className="w-4 h-4 mr-2 sm:hidden" /><span className="truncate">Programas</span>
                                </TabsTrigger>
                                <TabsTrigger value="oportunidades" className="min-h-[48px] sm:min-h-[44px] flex-shrink-0 sm:flex-1 min-w-[130px] sm:min-w-0 text-sm font-medium px-4 py-3 rounded-xl data-[state=active]:shadow-md transition-all touch-manipulation">
                                    <SearchIcon className="w-4 h-4 mr-2 sm:hidden" /><span className="truncate">Oportunidades</span>
                                </TabsTrigger>
                                <TabsTrigger value="infraestruturas" className="min-h-[48px] sm:min-h-[44px] flex-shrink-0 sm:flex-1 min-w-[145px] sm:min-w-0 text-sm font-medium px-4 py-3 rounded-xl data-[state=active]:shadow-md transition-all touch-manipulation">
                                    <ShieldCheckIcon className="w-4 h-4 mr-2 sm:hidden" /><span className="truncate">Inspecções</span>
                                </TabsTrigger>
                        ...
                        <TabsContent value="programas" className="mt-5 sm:mt-8 lg:mt-10 animate-in fade-in-50 duration-300">
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
                                {setor.programas.map((programa, index) => (
                                    <Card key={index} className="group relative flex flex-col hover:shadow-2xl hover:shadow-red-100/50 transition-all duration-300 hover:-translate-y-1 border-0 bg-white dark:bg-gray-800/90 overflow-hidden rounded-xl">
                                        <div className="h-1.5 sm:h-1 bg-gradient-to-r from-red-600 to-rose-600 w-full" />
                                        <CardHeader className="p-4 sm:p-5 lg:p-6 pb-3 sm:pb-4">
                                            <CardTitle className="flex items-start gap-3 text-base sm:text-lg group-hover:text-red-600 transition-all">
                                                <div className="p-2.5 sm:p-3 rounded-xl bg-red-50 dark:bg-red-900/30 text-red-600 group-hover:bg-red-600 group-hover:text-white transition-all shrink-0"><ShieldCheckIcon className="w-5 h-5" /></div>
                                        ...
                                        <CardContent className="flex-1 flex flex-col p-4 sm:p-5 lg:p-6 pt-0 space-y-4">
                                            <div className="space-y-2.5">
                                                <h4 className="font-semibold text-xs uppercase tracking-wider text-gray-500 flex items-center gap-2"><CheckCircleIcon className="w-4 h-4 text-green-500" />Informações</h4>
                                            ...
                                            <div className="pt-4 border-t border-gray-100 dark:border-gray-700 mt-auto">
                                                <Button className="w-full min-h-[48px] sm:min-h-[44px] bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white font-medium rounded-xl transition-all active:scale-[0.98] touch-manipulation" onClick={() => { setProgramaSelecionado(programa.titulo); setOpenInscricaoPrograma(true); }}>
                                                    <span>Saber Mais</span><ArrowRightIcon className="w-4 h-4 ml-2" />
                                                </Button>
                                            </div>
                                        </CardContent>
                                    </Card>
                        ...
                        <TabsContent value="infraestruturas" className="mt-5 sm:mt-8 lg:mt-10 animate-in fade-in-50 duration-300">
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
                                {setor.infraestruturas.map((infraestrutura, index) => (
                                    <Card key={index} className="group relative flex flex-col hover:shadow-2xl hover:shadow-orange-100/50 transition-all duration-300 hover:-translate-y-1 border-0 bg-white dark:bg-gray-800/90 overflow-hidden rounded-xl">
                                        <div className="h-1.5 sm:h-1 bg-gradient-to-r from-orange-500 to-amber-500 w-full" />
                                        <CardHeader className="p-4 sm:p-5 lg:p-6 pb-3 sm:pb-4">
                                            <div className="flex items-center justify-between mb-3 gap-2">
                                                <div className="p-2.5 sm:p-3 bg-orange-50 dark:bg-orange-900/30 rounded-xl text-orange-600 group-hover:bg-orange-600 group-hover:text-white transition-all shrink-0"><BuildingIcon className="w-5 h-5" /></div>
                                                <Badge variant="outline" className="border-orange-200 text-orange-700 text-xs px-2.5 py-1">Inspecção</Badge>
                                            </div>
                                            <CardTitle className="text-base sm:text-lg group-hover:text-orange-600 transition-all line-clamp-2">{infraestrutura.nome}</CardTitle>
                                            <p className="text-sm text-muted-foreground leading-relaxed mt-3">{infraestrutura.descricao}</p>
                                        </CardHeader>
                                        <CardContent className="flex-1 flex flex-col p-4 sm:p-5 lg:p-6 pt-0">
                                            <div className="pt-4 border-t border-gray-100 dark:border-gray-700 mt-auto">
                                                <Button variant="outline" className="w-full min-h-[48px] sm:min-h-[44px] border-orange-200 text-orange-700 hover:bg-orange-50 font-medium rounded-xl transition-all active:scale-[0.98] touch-manipulation" onClick={() => { setDetalheInfra(infraestrutura); setOpenDetalhes(true); }}>
                                                    <span>Ver Relatório</span><ArrowRightIcon className="w-4 h-4 ml-2" />
                                                </Button>
                                            </div>
                                        </CardContent>
                                    </Card>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
                                {setor.contactos.map((contacto, index) => (
                                    <Card key={index} className="group relative overflow-hidden hover:shadow-xl hover:shadow-purple-100/50 transition-all duration-300 hover:-translate-y-0.5 border-0 bg-white dark:bg-gray-800/90 rounded-xl">
                                        ...
                                        <CardHeader className="p-4 sm:p-5 lg:p-6 pb-3 sm:pb-4">
                                            <div className="flex items-center gap-3 mb-3"><div className="p-2.5 sm:p-3 bg-purple-100 dark:bg-purple-900/30 rounded-xl shrink-0"><PhoneIcon className="w-5 h-5 text-purple-600 dark:text-purple-400" /></div><Badge className="bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 text-xs px-2.5 py-1 hover:bg-purple-100">Contacto</Badge></div>
                                            <CardTitle className="text-base sm:text-lg line-clamp-2 group-hover:text-purple-600 transition-all">{contacto.responsavel}</CardTitle>
                                        </CardHeader>
                ...
                <DialogContent className="w-full max-w-2xl h-[100dvh] sm:h-auto sm:max-h-[85vh] rounded-none sm:rounded-2xl p-0 overflow-hidden border-0 sm:border gap-0">
                    <div className="flex flex-col h-full">
                        ...
                        <DialogHeader className="p-5 sm:p-6 pb-4 border-b border-gray-100 dark:border-gray-800 shrink-0 relative">
                            ...
                            <div className="flex items-start gap-3 pr-10 sm:pr-0"><div className="p-2.5 bg-orange-50 dark:bg-orange-900/30 rounded-xl text-orange-600 shrink-0"><BuildingIcon className="w-5 h-5" /></div><div><DialogTitle className="text-lg sm:text-xl font-semibold">{detalheInfra?.nome}</DialogTitle><DialogDescription className="text-sm mt-1">{detalheInfra?.descricao}</DialogDescription></div></div>
                        </DialogHeader>
                        {detalheInfra && (<div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-5">{detalheInfra.localizacao && (<div><h4 className="font-semibold text-sm mb-3 flex items-center gap-2"><MapPinIcon className="w-4 h-4 text-orange-500" />Localização</h4><p className="text-sm text-muted-foreground py-2 px-3 -mx-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">{detalheInfra.localizacao}</p></div>)}</div>)}
                        <DialogFooter className="p-5 sm:p-6 pt-4 border-t border-gray-100 dark:border-gray-800 shrink-0 bg-gray-50/50 dark:bg-gray-900/50"><Button variant="outline" onClick={() => setOpenDetalhes(false)} className="w-full sm:w-auto min-h-[48px] sm:min-h-[44px] font-medium rounded-xl transition-all touch-manipulation">Fechar</Button></DialogFooter>
                    </div>
                </DialogContent>
            </Dialog>
            <Footer />
