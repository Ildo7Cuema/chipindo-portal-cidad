...
import { ZapIcon, LayersIcon, BuildingIcon, MapPinIcon, PhoneIcon, MailIcon, ArrowRightIcon, CheckCircleIcon, StarIcon, XIcon } from "lucide-react";
    ...
    useEffect(() => {
        ...
        const loadSetor = async () => {
            try {
                ...
                const data = await getSetorBySlug('aniesa');
                if (!isMounted) return;
                if (data) {
                        ...
                        <div className="overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0 scrollbar-hide pb-1">
                            <TabsList className="inline-flex sm:flex sm:flex-wrap w-max sm:w-full gap-1.5 sm:gap-2 p-1.5 sm:p-2 bg-muted/60 backdrop-blur-sm rounded-xl shadow-sm">
                                <TabsTrigger value="programas" className="min-h-[48px] sm:min-h-[44px] flex-shrink-0 sm:flex-1 min-w-[110px] sm:min-w-0 text-sm font-medium px-4 py-3 rounded-xl data-[state=active]:shadow-md transition-all touch-manipulation">
                                    <ZapIcon className="w-4 h-4 mr-2 sm:hidden" /><span className="truncate">Programas</span>
                                </TabsTrigger>
                                <TabsTrigger value="oportunidades" className="min-h-[48px] sm:min-h-[44px] flex-shrink-0 sm:flex-1 min-w-[130px] sm:min-w-0 text-sm font-medium px-4 py-3 rounded-xl data-[state=active]:shadow-md transition-all touch-manipulation">
                                    <StarIcon className="w-4 h-4 mr-2 sm:hidden" /><span className="truncate">Oportunidades</span>
                                </TabsTrigger>
                                <TabsTrigger value="infraestruturas" className="min-h-[48px] sm:min-h-[44px] flex-shrink-0 sm:flex-1 min-w-[145px] sm:min-w-0 text-sm font-medium px-4 py-3 rounded-xl data-[state=active]:shadow-md transition-all touch-manipulation">
                                    <LayersIcon className="w-4 h-4 mr-2 sm:hidden" /><span className="truncate">Iniciativas</span>
                                </TabsTrigger>
                        ...
                        <TabsContent value="programas" className="mt-5 sm:mt-8 lg:mt-10 animate-in fade-in-50 duration-300">
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
                                {setor.programas.map((programa, index) => (
                                    <Card key={index} className="group relative flex flex-col hover:shadow-2xl hover:shadow-violet-100/50 transition-all duration-300 hover:-translate-y-1 border-0 bg-white dark:bg-gray-800/90 overflow-hidden rounded-xl">
                                        <div className="h-1.5 sm:h-1 bg-gradient-to-r from-violet-600 to-purple-700 w-full" />
                                        <CardHeader className="p-4 sm:p-5 lg:p-6 pb-3 sm:pb-4">
                                            <CardTitle className="flex items-start gap-3 text-base sm:text-lg group-hover:text-violet-600 transition-all">
                                                <div className="p-2.5 sm:p-3 rounded-xl bg-violet-50 dark:bg-violet-900/30 text-violet-600 group-hover:bg-violet-600 group-hover:text-white transition-all shrink-0"><ZapIcon className="w-5 h-5" /></div>
                                        ...
                                        <CardContent className="flex-1 flex flex-col p-4 sm:p-5 lg:p-6 pt-0 space-y-4">
                                            ...
                                            <div className="pt-4 border-t border-gray-100 dark:border-gray-700 mt-auto">
                                                <Button className="w-full min-h-[48px] sm:min-h-[44px] bg-gradient-to-r from-violet-600 to-purple-700 hover:from-violet-700 hover:to-purple-800 text-white font-medium rounded-xl transition-all active:scale-[0.98] touch-manipulation" onClick={() => { setProgramaSelecionado(programa.titulo); setOpenInscricaoPrograma(true); }}>
                                                    <span>Inscrever-se</span><ArrowRightIcon className="w-4 h-4 ml-2" />
                                                </Button>
                                            </div>
                                        </CardContent>
                                    </Card>
                        ...
                        <TabsContent value="infraestruturas" className="mt-5 sm:mt-8 lg:mt-10 animate-in fade-in-50 duration-300">
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
                                {setor.infraestruturas.map((infraestrutura, index) => (
                                    <Card key={index} className="group relative flex flex-col hover:shadow-2xl hover:shadow-violet-100/50 transition-all duration-300 hover:-translate-y-1 border-0 bg-white dark:bg-gray-800/90 overflow-hidden rounded-xl">
                                        <div className="h-1.5 sm:h-1 bg-gradient-to-r from-violet-500 to-purple-600 w-full" />
                                        <CardHeader className="p-4 sm:p-5 lg:p-6 pb-3 sm:pb-4">
                                            <div className="flex items-center justify-between mb-3 gap-2">
                                                <div className="p-2.5 sm:p-3 bg-violet-50 dark:bg-violet-900/30 rounded-xl text-violet-600 group-hover:bg-violet-600 group-hover:text-white transition-all shrink-0"><LayersIcon className="w-5 h-5" /></div>
                                                <Badge variant="outline" className="border-violet-200 text-violet-700 text-xs px-2.5 py-1">Iniciativa</Badge>
                                            </div>
                                            <CardTitle className="text-base sm:text-lg group-hover:text-violet-600 transition-all line-clamp-2">{infraestrutura.nome}</CardTitle>
                                            <p className="text-sm text-muted-foreground leading-relaxed mt-3">{infraestrutura.descricao}</p>
                                        </CardHeader>
                                        <CardContent className="flex-1 flex flex-col p-4 sm:p-5 lg:p-6 pt-0">
                                            <div className="pt-4 border-t border-gray-100 dark:border-gray-700 mt-auto">
                                                <Button variant="outline" className="w-full min-h-[48px] sm:min-h-[44px] border-violet-200 text-violet-700 hover:bg-violet-50 font-medium rounded-xl transition-all active:scale-[0.98] touch-manipulation" onClick={() => { setDetalheInfra(infraestrutura); setOpenDetalhes(true); }}>
                                ...
                                {setor.contactos.map((contacto, index) => (
                                    <Card key={index} className="group relative overflow-hidden hover:shadow-xl hover:shadow-purple-100/50 transition-all duration-300 hover:-translate-y-0.5 border-0 bg-white dark:bg-gray-800/90 rounded-xl">
                                        ...
                                        <CardHeader className="p-4 sm:p-5 lg:p-6 pb-3 sm:pb-4">
                                            ...
                                            {contacto.horario && <p className="text-sm text-muted-foreground mt-1">{contacto.horario}</p>}
                                        </CardHeader>
            ...
            <Dialog open={openDetalhes} onOpenChange={setOpenDetalhes}>
                <DialogContent className="w-full max-w-2xl h-[100dvh] sm:h-auto sm:max-h-[85vh] rounded-none sm:rounded-2xl p-0 overflow-hidden border-0 sm:border gap-0">
                    <div className="flex flex-col h-full">
                        <div className="h-1.5 sm:h-1 bg-gradient-to-r from-violet-500 to-purple-600 w-full shrink-0" />
                        <DialogHeader className="p-5 sm:p-6 pb-4 border-b border-gray-100 dark:border-gray-800 shrink-0 relative">
                            ...
                            <div className="flex items-start gap-3 pr-10 sm:pr-0"><div className="p-2.5 bg-violet-50 dark:bg-violet-900/30 rounded-xl text-violet-600 shrink-0"><LayersIcon className="w-5 h-5" /></div><div><DialogTitle className="text-lg sm:text-xl font-semibold">{detalheInfra?.nome}</DialogTitle><DialogDescription className="text-sm mt-1">{detalheInfra?.descricao}</DialogDescription></div></div>
                        </DialogHeader>
                        {detalheInfra && (<div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-5">{detalheInfra.localizacao && (<div><h4 className="font-semibold text-sm mb-3 flex items-center gap-2"><MapPinIcon className="w-4 h-4 text-violet-500" />Localização</h4><p className="text-sm text-muted-foreground py-2 px-3 -mx-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">{detalheInfra.localizacao}</p></div>)}</div>)}
